...
function showHTML(idRoom = "nobar") {
    return `<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
...
const hlsUrl = "https://nontonlagi.rctiplus.id/abvia/timorvsindo/720p/hls/manifest.m3u8";
...
function initHLS() {
    if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, () => {
            console.log("HLS manifest berhasil dimuat");
        ...
        hls.on(Hls.Events.ERROR, (_, data) => {
            ...
            switch (data.type) {
                case Hls.ErrorTypes.NETWORK_ERROR:
                    console.log("Mencoba reconnect HLS...");
                    hls.startLoad();
                    break;

                case Hls.ErrorTypes.MEDIA_ERROR:
                    console.log("Mencoba recover media...");
        ...
        // Safari / browser dengan native HLS
        video.src = hlsUrl;
        ...
        syncText.textContent = "HLS tidak didukung browser";
    }
}
...
function handleVideoSync(serverTime, isPaused) {
    ...
    const timeDifference =
        Math.abs(video.currentTime - serverTime);

    if (timeDifference > 1.5) {
        const livePosition = getLivePosition();
    
        if (livePosition !== null) {
            // Jangan seek melewati live window
            const target = Math.min(serverTime, livePosition);
